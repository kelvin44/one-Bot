...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] Scan kode qr lu anjim!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @kelvin_exploit`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @kelvin_exploit`)
})
...
conn.sendMessage(id, 'Jangan manggil" creator ku! https://wa.me/6285212830930' ,MessageType.text);
}
else if (text == 'bot'){
conn.sendMessage(id, 'meoww:3' ,MessageType.text);
}
...
else if (text == 'Haturnuhun'){
conn.sendMessage(id, 'Sami sami, semoga harimu menyenangkan :)' ,MessageType.text);
}
