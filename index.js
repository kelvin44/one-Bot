...
conn.sendMessage(id, 'Owner kelvin paling ganteng:v wa.me/6285212830930' ,MessageType.text);
}
else if (text == '.creator'){
conn.sendMessage(id, 'Creator One Bot nih boss wa.me/6285212830930' ,MessageType.text);
}
...
conn.sendMessage(id, 'Lu babi anjg-_' ,MessageType.text);
}
...
if (text.includes(".katakotor")){
	const alay = text.split(".katakotor")[1]
	axios.get(`https://api.terhambar.com/bpk?kata=${katakotor}`).then ((res) =>
		{ let hasil = `${res.data.text}`
		conn.sendMessage(id, hasil, MessageType.text)
	})
}
