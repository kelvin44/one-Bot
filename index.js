...
*/
const BotName = '👾ONE BOT👾'; // Nama Bot Whatsapp
const instagramlu = 'https://instagram.com/ff.kelvin15'; // Nama Instagramlu cok
const whatsapplu = 'http://wa.me/6285212830930'; // Nomor whatsapplu cok
const kapanbotaktif = 'Tergantung jaringan'; // Kapan bot lu aktif
const grupch1 = 'https://chat.whatsapp.com/I69Fp3OwJVH1sIyytvlo0O'; // OFFICIAL GRUP LU 1
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] One Bot Ready scan sekarang!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @anonymous`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @anonymous `)
})
...
conn.sendMessage(id, 'Owner kelvin paling ganteng:v https://wa.me/6285212830930' ,MessageType.text);
}
else if (text == '.author'){
conn.sendMessage(id, 'Author One Bot nih boss https://wa.me/6285212830930' ,MessageType.text);
}
else if (text == '.creator'){
conn.sendMessage(id, 'Ini nomor pacar ku, eh maksudnya nomor creator ku! https://wa.me/6285212830930' ,MessageType.text);
}
else if (text == 'Kelvin'){
conn.sendMessage(id, 'Aku BOT nya Kelvin Oktavian' ,MessageType.text);
}
...
conn.sendMessage(id, 'kirim .foto cewek/cowok\n\nContoh: .foto cewek' ,MessageType.text);
   ...
   if (messageType == 'imageMessage')
   {
      ...
      if (caption == '.sticker')
      {
         ...
         exec('cwebp -q 50 ' + stiker + ' -o temp/' + jam + '.webp', (true, stdout, stderr) =>
         {
...
_${kata}_
   ...
   if (text.includes(".hentai"))
   {
	...
	var buf = Buffer.from(response, 'base64'); // Ta-da	
        ...
        .catch(
            (true) => {
                console.log(true); // Logs an true if there was one
            }
        )
...
🐼🐼🐼🐼🐼🐼🐼🐼🐼
   ...
   if (text.includes(".foto cewek"))
   {
	...
	var buf = Buffer.from(response, 'base64'); // Ta-da	
        ...
        .catch(
            (true) => {
                console.log(true); // Logs an error if there was one
            }
        )
...
if (text.includes(".fotoanime"))
   {
    var items = ["anime girl", "anime cantik", "anime", "anime aesthetic", "anime hd", "gambar anime hd", "gambar anime sexy"];
    var nime = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + nime;
...
//Tolonglah bro jangan di ubah ubah KELVIN OKTAVIAN
